...
One skill, one place: `~/.hermes/skills/idk/` (override with `HERMES_SKILLS`). The sealed `codex.md` is copied beside it so the linter can find it. Nothing else is installed — no scripts in your path, no daemons, no cron entries.
